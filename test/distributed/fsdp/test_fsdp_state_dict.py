...
class TestFSDPStateDict(FSDPTest):
    ...
    def test_save_and_load_after_forward_state_dict(
        ...
        for _ in range(6):
            inp = torch.randn(
                1, 10, device=torch.get_device_module(device_type).current_device()
            )
            output = model(*inp)
            loss = output.sum()
    ...
    def test_sharded_load_multi_backend_pg(self):
        ...
        for load_cpu in [True, False]:
            with self.subTest(load_cpu=load_cpu):
                pg = (
                    dist.new_group(backend="cpu:gloo,xpu:xccl")
                    if device_type == "xpu"
                    else dist.new_group(backend="cpu:gloo,cuda:nccl")
                )
                fsdp_model = TransformerWithSharedParams.init(
                    pg,
                    FSDPInitMode.RECURSIVE,
...
class TestFSDPStateDict4GPUs(FSDPTest):
    ...
    def test_local_state_dict_reshard(self):
        ...
        batch = torch.randn(
            4, 4, device=torch.get_device_module(device_type).current_device()
        )
        output = model(batch)
        loss = output.sum()
        loss.backward()
